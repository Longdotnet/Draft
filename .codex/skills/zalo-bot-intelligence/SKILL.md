...
- Do not equate one logical bridge route with one Zalo provider attempt. Pagination, batch enrichment, semantic fallback and optional provider lookups can fan one logical request into many upstream calls; every concrete SDK call that can touch Zalo must cross the account-scoped pacing/cooldown boundary, while logical read coalescing stays outside that serialized provider queue to avoid nested-governor deadlocks.
