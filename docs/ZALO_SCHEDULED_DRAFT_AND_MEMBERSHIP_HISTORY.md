...
Every scheduled time is derived from that match's own date in Vietnam. For example, after the September 24 match has been drafted, a September 25 tick with no match that day cannot draft the September 24 match again or draft a September 26 match early. One completed scheduled run is retained per session, even if the policy is edited. A previously failed result notification may be retried on a later day; such a notification reports the stored result and never invokes draft again. The day-boundary behavior is covered by an executable regression test.
