...
public sealed partial class ZaloOverbookService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var normalized = ZaloMatchBriefPolicy.Normalize(incoming.Content);
        var references = sessions
            .Select(session => new ZaloSessionReference(session.Id, session.Name, session.StartTime))
            .ToList();
        var matchedIds = ZaloConversationCore.ResolveSessionReference(normalized, references, now);
            ...
            (selected.Status is SessionStatus.Setup or SessionStatus.CaptainSelection))
        {
            ...
            if (!synced.IsSuccess)
            {
                await SendDraftReplyAsync(
                    ...
                    groupId,
                    incoming,
                    $"Tui chưa refresh được poll của {selected.Name}, nên không lấy snapshot cũ trả như dữ liệu hiện tại nha. Chờ poll sync lại được rồi hỏi tui lại.",
                    [],
