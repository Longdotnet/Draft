...
public sealed partial class ZaloOverbookService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            await SendProfileUpdateReplyAsync(sessions[0], incoming,
        ...
        if (duplicateSession is not null)
        {
            ...
            await SendProfileUpdateReplyAsync(conflictedSession, incoming,
        ...
        {
            await SendProfileUpdateReplyAsync(targetSessions[0], incoming,
        ...
        var payload = new ZaloProfileUpdateConversationPayload(
            ...
            targetUid, targetDisplay, parsed.Gender, parsed.Role, parsed.Level, incoming.MessageId);
        ...
        await SendProfileUpdateReplyAsync(authorizedSessions[0], incoming,
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (matches.Count != 1)
        {
            ...
            await SendProfileUpdateReplyAsync(candidates[0], incoming,
        ...
        if (!await CanOperateProfileUpdateAsync(selected, incoming.SenderId))
        {
            db.ZaloBotConversationStates.Remove(state);
            await db.SaveChangesAsync(cancellationToken);
            await SendProfileUpdateReplyAsync(selected, incoming,
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            await SendProfileUpdateReplyAsync(session, incoming,
                currentRows.Count == 0
        ...
        var readinessText = ZaloProfileUpdateReadinessCopy.Build(readiness);
        await SendProfileUpdateReplyAsync(session, incoming,
            $"Đã cập nhật {player.DisplayName} cho {session.Name}: {FormatProfileGender(player.Gender)}, {FormatProfileRole(player.Role)}, {FormatProfileLevel(player.Level)}.{readinessText}",
    ...
    private static ZaloNaturalProfileValues ParseExplicitProfileUpdateValues(string? content)
    {
        var parsed = ZaloNaturalProfileReplyParser.Parse(
            content, missingGender: true, missingRole: true, missingLevel: true, repliedToPrompt: true);
