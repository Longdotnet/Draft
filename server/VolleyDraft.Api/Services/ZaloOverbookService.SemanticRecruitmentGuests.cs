...
public sealed partial class ZaloOverbookService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        var turn = await ResolveSemanticGuestTurnContextAsync(connectionId, groupId, incoming.MessageId, senderId, cancellationToken);
        ...
        var recentIds = await LoadRecentGuestContextMessageIdsAsync(connectionId, groupId, actionSettings.MaxContextMessages, cancellationToken);
        ...
        var plan = await planner.PlanAsync(connectionId, groupId, incoming.Content, conversation, snapshot, actionSettings, cancellationToken);
        var aiCalled = !IsSemanticGuestTechnicalFallback(plan.Reason);
        if (IsSemanticGuestTechnicalFallback(plan.Reason)) plan = BuildSemanticGuestFallbackPlan(incoming.Content, snapshot);
        ...
        if (plan.Action == ZaloSemanticGuestActionKind.None)
        {
            ...
            {
                await MarkSemanticGuestTurnWithoutReplyAsync(connectionId, groupId, incoming, "guest_semantic_noop", aiCalled, cancellationToken);
                return true;
            }
            return false;
        }
        ...
        if (!validation.Accepted)
        {
            ...
            await SendSemanticGuestReplyAsync(connectionId, accountId, botName, groupId, incoming, turn.Session.Id,
        ...
        try
        {
            ...
            var extended = await TryExecuteExtendedSemanticGuestActionAsync(
                turn.Session, senderId, incoming.SenderName, incoming.MessageId, turn.RecruitmentMessageId,
            ...
            if (extended is not null)
            {
                ...
                await SyncSemanticGuestConversationStateAsync(
                    groupId, senderId, turn.Session.Id, incoming.MessageId, turn.RecruitmentMessageId, cancellationToken);
            }
            else if (validation.Action == ZaloSemanticGuestActionKind.AddGuests)
            {
                ...
                var preview = await BuildSemanticGuestMutationPreviewAsync(turn.Session.Id, validation.Quantity, cancellationToken);
                logger.LogInformation(
                    ...
                    turn.Session.Id, senderId, preview.BeforeEffectiveSlots, preview.Capacity, validation.Quantity,
                    preview.AdmitCount, preview.WaitlistCount, preview.AfterEffectiveSlots);

                var command = new ZaloRecruitmentGuestCommand(
                    ZaloRecruitmentGuestCommandKind.Add,
                    validation.Quantity,
                    validation.Items.Select(item => new ZaloRecruitmentGuestSpec(item.DisplayName, item.Gender, item.Role, item.Level)).ToArray());
                var result = await service.AddAsync(
                    turn.Session, senderId, FriendlySponsorName(incoming.SenderName, senderId), incoming.MessageId,
                    turn.RecruitmentMessageId, command, cancellationToken);
                ...
                foreach (var item in validation.Items)
                {
                    ...
                    if (result.NeedsClarification)
                    {
                        await SendSemanticGuestReplyAsync(connectionId, accountId, botName, groupId, incoming, turn.Session.Id,
                            ...
                            "guest_semantic_update_clarification", aiCalled, cancellationToken);
                        return true;
                    }
            ...
            else
            {
                ...
                foreach (var item in validation.Items)
                {
                    var result = await service.CancelAsync(
                        turn.Session, senderId,
                        new ZaloRecruitmentGuestCommand(ZaloRecruitmentGuestCommandKind.Cancel, SponsorSequence: item.SponsorSequence),
                        cancellationToken);
                    if (result.NeedsClarification)
                    {
                        await SendSemanticGuestReplyAsync(connectionId, accountId, botName, groupId, incoming, turn.Session.Id,
                            result.Clarification ?? "Tui chưa xác định chắc guest nào nghỉ.",
                            "guest_semantic_cancel_clarification", aiCalled, cancellationToken);
                        return true;
                    }
                    changed.AddRange(result.Changed);
                }
        ...
        {
            logger.LogWarning(exception, "Semantic guest execution failed Session={SessionId} Message={MessageId}", turn.Session.Id, incoming.MessageId);
            await SendSemanticGuestReplyAsync(connectionId, accountId, botName, groupId, incoming, turn.Session.Id,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var relation = await new ZaloMessageGraphStore(db).LoadRelationAsync(connectionId, groupId, incomingMessageId, cancellationToken);
        if (relation?.RelationType == "ReplyTo" && !string.IsNullOrWhiteSpace(relation.ToMessageId))
        {
            var anchor = await db.ZaloGroupMessages.AsNoTracking().SingleOrDefaultAsync(item =>
                item.ZaloConnectionId == connectionId && item.GroupId == groupId &&
                item.MessageId == relation.ToMessageId && item.IsFromBot, cancellationToken);
        ...
        {
            var active = await new ZaloConversationStateV2Store(db).LoadActiveAsync(groupId, senderId, cancellationToken);
        ...
        var session = await db.MatchSessions.AsNoTracking().Include(item => item.ZaloConnection).SingleOrDefaultAsync(item =>
            item.Id == sessionId && item.ZaloConnectionId == connectionId && item.ZaloGroupId == groupId &&
            item.BotEnabled && item.ZaloConnection != null &&
            (item.Status == SessionStatus.Setup || item.Status == SessionStatus.CaptainSelection) &&
            item.StartTime != null && item.StartTime > now, cancellationToken);
        if (session is null) return null;

        var guests = await db.ZaloGuestReservations.AsNoTracking().Where(item =>
            item.SessionId == session.Id && item.SponsorZaloUserId == senderId &&
            ...
            .OrderBy(item => item.SponsorSequence).ToListAsync(cancellationToken);
    ...
    private async Task<ZaloSemanticGuestGroundingSnapshot> BuildSemanticGuestSnapshotAsync(
        ...
        CancellationToken cancellationToken)
    {
        var readiness = await new ZaloDraftReadinessService(db).BuildAsync(turn.Session.Id, cancellationToken: cancellationToken);
        var utc = DateTimeOffset.UtcNow;
        var snapshot = new ZaloSemanticGuestGroundingSnapshot(
            turn.Session.Id, turn.Session.Name, turn.Session.StartTime,
            ...
            senderId, FriendlySponsorName(senderName, senderId), turn.AnchorKind, turn.RecruitmentMessageId,
            turn.ExistingGuests.Select(item => new ZaloSemanticGuestGroundingGuest(
                item.Id, item.SponsorSequence, item.DisplayName, item.Gender, item.Level, item.Role, item.Status.ToString())).ToArray(),
            turn.PendingMissingFields, utc, utc.ToOffset(TimeSpan.FromHours(7)));
        ...
        CancellationToken cancellationToken)
    {
        ...
        return new(readiness.EffectiveSlotCount, readiness.Capacity, admitted,
            Math.Max(0, quantity - admitted), readiness.EffectiveSlotCount + admitted);
    }
        ...
        CancellationToken cancellationToken)
    {
        await ZaloGuestReservationSchemaPatch.EnsureAsync(db, cancellationToken);
        var guests = await db.ZaloGuestReservations.AsNoTracking().Where(item =>
            item.SessionId == sessionId && item.SponsorZaloUserId == senderId &&
            (item.Status == ZaloGuestReservationStatus.Active ||
             item.Status == ZaloGuestReservationStatus.Waitlisted ||
             item.Status == ZaloGuestReservationStatus.Tentative))
            .OrderBy(item => item.SponsorSequence).ToListAsync(cancellationToken);
        var missing = guests.Where(item => item.Gender is null)
            .Select(item => $"gender:#{item.SponsorSequence}").ToList();
        missing.AddRange(guests.Where(item => item.Status == ZaloGuestReservationStatus.Tentative)
            .Select(item => $"confirmation:#{item.SponsorSequence}"));
        var missingArray = missing.Distinct(StringComparer.Ordinal).ToArray();
        ...
        var minutes = Math.Clamp(configuration.GetValue("ZaloBot:DraftAutopilot:GuestProfileConversationMinutes", 60), 10, 180);
        await store.SaveActiveAsync(
            groupId, senderId, SemanticGuestConversationIntent,
            ...
            JsonSerializer.Serialize(missingArray),
            JsonSerializer.Serialize(guests.Select(item => new
            {
                item.Id, item.SponsorSequence, item.DisplayName, item.Gender, item.Level, item.Role,
                Status = item.Status.ToString()
            })),
            sourceMessageId, lastMessageId, DateTimeOffset.UtcNow.AddMinutes(minutes), cancellationToken);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var rows = await db.ZaloGroupMessages.AsNoTracking()
            .Where(item => item.ZaloConnectionId == connectionId && item.GroupId == groupId && item.ReceivedAt >= cutoff)
            .Select(item => new { item.MessageId, item.ReceivedAt }).Take(160).ToListAsync(cancellationToken);
        return rows.OrderBy(item => item.ReceivedAt).Select(item => item.MessageId)
            .TakeLast(Math.Clamp(maxContextMessages * 3, 12, 60)).ToArray();
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var text = $"{label} {body}";
        IReadOnlyList<BridgeOutgoingMention> mentions = senderId.Length == 0 ? [] : [new BridgeOutgoingMention(senderId, 0, label.Length)];
        var idempotencyKey = $"guest-semantic:{connectionId}:{incoming.MessageId}";
        var send = await bridge.SendGroupMessageAsync(accountId, groupId, text, mentions, idempotencyKey: idempotencyKey);
        var providerReplyId = NormalizeProviderMessageId(send.MessageId);
        var persistedReplyId = providerReplyId ?? $"local:{idempotencyKey}";
        await EnsureV2OutboundMessageAsync(connectionId, groupId, persistedReplyId, accountId, botName, text, cancellationToken);

        var outbound = await db.ZaloGroupMessages.SingleOrDefaultAsync(item =>
            item.ZaloConnectionId == connectionId && item.GroupId == groupId && item.MessageId == persistedReplyId, cancellationToken);
        ...
        if (providerReplyId is not null)
            await new ZaloMessageGraphStore(db).RememberOutboundAsync(connectionId, groupId, providerReplyId, incoming.MessageId, cancellationToken);
    ...
    private static ZaloSemanticGuestPlan BuildSemanticGuestFallbackPlan(string content, ZaloSemanticGuestGroundingSnapshot snapshot)
    {
        ...
        if (command is not null)
        {
            if (command.Kind == ZaloRecruitmentGuestCommandKind.Add)
            {
                return new ZaloSemanticGuestPlan(
                    ZaloSemanticGuestActionKind.AddGuests, 1, command.Quantity, 1,
                    (command.Guests ?? []).Select(item => new ZaloSemanticGuestPlanItem(
                        string.Empty, null, null, item.DisplayName, item.DisplayName is null ? 0 : 1,
                        item.Gender, item.Gender is null ? 0 : 1, item.Level, item.Level is null ? 0 : 1,
                        item.Role, item.Role is null ? 0 : 1, 1)).ToArray(),
                    false, string.Empty, "semantic_guest_deterministic_fallback");
            }

            var candidates = snapshot.ExistingGuests;
            ZaloSemanticGuestGroundingGuest? target = null;
            if (command.SponsorSequence is { } seq) target = candidates.SingleOrDefault(item => item.SponsorSequence == seq);
            else if (!string.IsNullOrWhiteSpace(command.GuestReference))
            {
                ...
                var matches = candidates.Where(item => ZaloBotIntelligence.Normalize(item.DisplayName).Contains(reference, StringComparison.Ordinal)).Take(2).ToArray();
                if (matches.Length == 1) target = matches[0];
            }
            else if (candidates.Count == 1) target = candidates[0];

            if (command.Kind == ZaloRecruitmentGuestCommandKind.UpdateProfile && target is not null)
                return new ZaloSemanticGuestPlan(
                    ZaloSemanticGuestActionKind.UpdateGuestProfiles, 1, 1, 1,
                    [new ZaloSemanticGuestPlanItem(
                        $"#{target.SponsorSequence}", target.ReservationId, target.SponsorSequence,
                        command.RenameTo, command.RenameTo is null ? 0 : 1,
                        command.Gender, command.Gender is null ? 0 : 1,
                        command.Level, command.Level is null ? 0 : 1,
                        command.Role, command.Role is null ? 0 : 1, 1)],
                    false, string.Empty, "semantic_guest_deterministic_fallback");
            if (command.Kind == ZaloRecruitmentGuestCommandKind.Cancel && target is not null)
                return new ZaloSemanticGuestPlan(
                    ZaloSemanticGuestActionKind.CancelGuests, 1, 1, 1,
                    [new ZaloSemanticGuestPlanItem($"#{target.SponsorSequence}", target.ReservationId, target.SponsorSequence,
        ...
        if (missingGender.Length == 1 && normalized is "nam" or "nu")
        {
            ...
            return new ZaloSemanticGuestPlan(
                ZaloSemanticGuestActionKind.UpdateGuestProfiles, 1, 1, 1,
                [new ZaloSemanticGuestPlanItem($"#{target.SponsorSequence}", target.ReservationId, target.SponsorSequence,
                    null, 0, normalized == "nam" ? PlayerGender.Male : PlayerGender.Female, 1, null, 0, null, 0, 1)],
                false, string.Empty, "semantic_guest_short_profile_fallback");
        }
        if (missingGender.Length == 2 && normalized is "nam nu" or "nu nam")
        {
            var firstMale = normalized == "nam nu";
            return new ZaloSemanticGuestPlan(
                ZaloSemanticGuestActionKind.UpdateGuestProfiles, 1, 2, 1,
                [
                    new ZaloSemanticGuestPlanItem($"#{missingGender[0].SponsorSequence}", missingGender[0].ReservationId, missingGender[0].SponsorSequence,
                        null, 0, firstMale ? PlayerGender.Male : PlayerGender.Female, 1, null, 0, null, 0, 1),
                    new ZaloSemanticGuestPlanItem($"#{missingGender[1].SponsorSequence}", missingGender[1].ReservationId, missingGender[1].SponsorSequence,
    ...
    private static bool IsSemanticGuestTechnicalFallback(string reason) => reason is
        "semantic_guest_disabled" or "semantic_guest_ai_not_configured" or "semantic_guest_budget_exhausted" or
        "semantic_guest_ai_error" or "semantic_guest_malformed_json";
        ...
        try
        {
            ...
            return document.RootElement.ValueKind == JsonValueKind.Array
                ? document.RootElement.EnumerateArray().Where(item => item.ValueKind == JsonValueKind.String)
                    .Select(item => item.GetString() ?? string.Empty).Where(item => item.Length > 0).Take(20).ToArray()
                : [];
        }
    ...
    private static string BuildSemanticGuestAddReply(ZaloGuestSignupMutationResult result)
    {
        ...
        var state = waiting == 0 ? $"giữ {admitted} guest vào roster"
            : admitted == 0 ? $"roster đang full nên đưa {waiting} guest vào danh sách chờ"
        ...
        var ask = missing.Length == 0 ? string.Empty
        ...
        var full = result.AfterEffectiveSlots >= result.Capacity ? " Kèo đủ slot rồi nên tui dừng réo tuyển thêm." : string.Empty;
    ...
    private static string BuildSemanticGuestProfileReply(string sessionName, IEnumerable<ZaloGuestReservation> changed)
    {
        ...
        return $"Đã cập nhật guest {sessionName}: {string.Join(", ", items.Select(FormatSemanticGuest))} 👌";
    }
