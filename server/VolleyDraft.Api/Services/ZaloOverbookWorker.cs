    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            try
            {
                ...
                // Human-comfort lane: once NPC has asked a verified member for missing
                // profile data, their ordinary short reply ("nam", "thủ", "mới chơi"...)
                // should be picked up quickly without forcing @bot or command syntax.
                // Keep this light lane frequent while the heavier orchestration remains
                // on the existing two-minute cadence.
                var profileReplies = await overbook.ProcessMissingProfileRepliesDueAsync(stoppingToken);
                if (profileReplies > 0)
                    logger.LogInformation("Missing-profile conversation cycle handled {HandledCount} reply/replies", profileReplies);

                var cycleNow = DateTimeOffset.UtcNow;
                if (cycleNow >= nextHeavyCycleAt)
                {
                    nextHeavyCycleAt = cycleNow.AddMinutes(2);
